...
<template>
  <v-layout column>
    <img src="/img/ressources/logoLynxter-dark.png" width="350px" style="margin:50px auto">
    <h2 class="display-1" style="text-align: center; background: #616161; font-family:'GTAmericaExpandedRegular', sans-serif !important; text-transform: uppercase;letter-spacing: 0.1rem !important">
      {{ selectedMachine === '[default]' ? $t('dialog.connect.title') : 'Choose a material' }}
    </h2>
    <!--v-layout row wrap>
      <v-btn class="material_btn"><img src="/img/ressources/Medium_universe_FIL.svg" width="90%">Filament</v-btn>
      <v-btn class="material_btn"><img src="/img/ressources/Medium_universe_LIQ.svg" width="90%">Liquid</v-btn>
      <v-btn class="material_btn"><img src="/img/ressources/Medium_universe_PAS.svg" width="90%">Paste</v-btn>
    </v-layout-->
    ...
    <v-spacer></v-spacer>
    <!--video :poster="videoPlaylist[curVideo].poster" style="width: 100%;" @timeupdate="videoUpdate" @ended="videoEnded" @progress="videoBuffer" muted preload autoplay>
      <source :src="videoPlaylist[curVideo].src" type="video/mp4">
    </video>
    <div id="video-controls">
      <v-btn icon id="previous" @click="previousTrack" style="margin: 0 5px 5px 0;"><v-icon>skip_previous</v-icon></v-btn>
      <v-btn icon id="play-pause" @click="playPause" style="margin: 0 5px 5px 0;"><v-icon>{{ playIcon }}</v-icon></v-btn>
      <v-btn icon id="next" @click="nextTrack" style="margin: 0 5px 5px 0;"><v-icon>skip_next</v-icon></v-btn>
      <v-slider min="0" max="100" v-model="buffer" :buffer-value="bufferValue" id="seek-bar" style="width: 70%; display: inline-flex" @change="seekChange(buffer)" @mousedown="seekMouseDown" @mouseup="seekMouseUp"></v-slider>
				...
				for (var i = 0; i < len; i++){
					...
					response.forEach(
						function (tool) {
							...
							if (that.tools[material] === undefined) that.tools[material] = [];
