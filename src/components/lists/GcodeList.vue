		...
		<v-card :class="{local: isLocal}" class="card">
			...
			<v-card-text class="pa-0" v-show="loading || filelist.length || !isRootDirectory" v-if="!isLocal">
				...
				<v-list class="pt-0" v-bind:class="{'v-list--dense': !isLocal}">
					...
					<v-list-tile v-for="(item) in filelist.filter((item,index) => index < 16)" :key="item.name" @click="onItemClick(item)" v-tab-control>
			...
			<v-alert :value="!filelist.length" color="darkgray"><!-- -->
						...
						if (this.getTool !== undefined && this.getTool !== "") {
							if (directory ===  Path.gcodes && this.getTool.substr(0,3).toUpperCase() !== "DEB") {
								if (this.getTool.substr(0,3).toUpperCase() == "CAL") {
									directory += "/" + this.getTool.substr(0,3)
								} else {
