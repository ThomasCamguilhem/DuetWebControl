...
def layer_changed(timelapse_folder, webcam_url, webcam_http_auth, webcam_https_verify, resolution, file=sys.stdout):
	if webcam_url == "":
		...
		for video in dev:
			...
			if out.returncode == 0:
				break
		r = requests.get("http://localhost:8080/?action=snapshot", auth=webcam_http_auth, verify=webcam_https_verify, timeout=5, stream=True)
	else :
...
def firmware_monitor( duet_host, file, inter, onLayer, minf, maxf, resolution, snapshot_folder, target, webcam_http_auth, webcam_https_verify, webcam_url):
	...
	while True:
		try:
			...
			#layer_changed(snapshot_folder, webcam_url, webcam_http_auth, webcam_https_verify, resolution, file=file)
			log_print("Connection established.", file=file)
			#ffmpeg = subprocess.Popen("rm -R '"+snapshot_folder+"*.jpg'", stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
			...
			while True:
				...
				if status == 'I' and timelapse_folder:
					...
					if onLayer :
						...
						nbImg = int(stdout)
						cfps = nbImg/target
						minfps = max(cfps, minf)
						finfps = min(minfps, maxf)
						cmdline = "ffmpeg -r " + str(finfps) + " -y -pattern_type glob -i '" + timelapse_folder + "/*.jpg' -c:v libx264 -pix_fmt yuvj420p '" +timelapse_folder + ".mp4'"
						log_print("Generating timelapse.", file=file)
						ffmpeg = subprocess.Popen(cmdline, stdout=subprocess.PIPE, shell=True)
						stdout, stderr = ffmpeg.communicate()
						ffmpeg = subprocess.Popen("cp -p 'ls -dtr1 \"" + timelapse_folder + "/*.jpg\"  | tail  -1' '" +timelapse_folder + ".jpg'", stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
					...
					if inter > 0:
						...
						minfps = max(cfps, minf)
						finfps = min(minfps, maxf)
						cmdline = "ffmpeg -r " + str(computedFr) + " -y -pattern_type glob -i '" + interval_folder + "/*.jpg' -c:v libx264 -pix_fmt yuvj420p '" + interval_folder + ".mp4'"
						...
						stdout, stderr = ffmpeg.communicate()
						ffmpeg = subprocess.Popen("cp -p 'ls -dtr1 \"" + interval_folder + "/*.jpg\"  | tail  -1' '" + interval_folder + ".jpg'", stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
						log_print("Cleaning up ressources.", file=file)
...
if __name__ == "__main__":
	...
	while x < len(sys.argv) :
		if sys.argv[x] == "-a" or sys.argv[x] == "--auth":
			...
			x = x+1
			minf = int(sys.argv[x].split(':')[0])
			maxf = int(sys.argv[x].split(':')[1])
